...
with tab_piper:
    ...
    st.caption("Diagrama generado con la librería [WQChartPy](https://github.com/jyangfsu/WQChartPy).")

    piper_pct = piper_mod.calcular_porcentajes_piper(datos["hidroquimica"])
    n_muestras = len(piper_pct)
    n_pozos_piper = piper_pct["Cod UK"].nunique()
    ...
    if piper_pct.empty:
        ...
        piper_pct = piper_pct.merge(pozos[["Cod UK", "acuifero"]], on="Cod UK", how="left")
        piper_pct["acuifero"] = piper_pct["acuifero"].fillna("Sin clasificar")
        piper_pct = piper_pct[piper_pct["acuifero"].isin(acuifero_sel)]

        if piper_pct.empty:
            st.info("No hay muestras con set iónico completo para el/los acuífero(s) seleccionado(s) en el filtro.")
        else:
            # Armamos el DataFrame en el formato que espera wqchartpy: columnas de
            # iones en mg/L (Ca, Mg, Na, K, HCO3, CO3, Cl, SO4) + estilo por fila
            # (Label = leyenda, Color, Marker, Size, Alpha). La alcalinidad
            # carbonatada (CO3) casi no está medida en la base; se usa cuando
            # está disponible y se asume 0 (agua a pH neutro) si falta.
            df_wq = pd.DataFrame({
                "Ca": piper_pct["Ca2+ (mg Ca2+/L)"],
                "Mg": piper_pct["Mg2+ (mg Mg2+/L)"],
                "Na": piper_pct["Na+ (mg Na+/L)"],
                "K": piper_pct["K+(mg K+/L)"],
                "HCO3": piper_pct["HCO3 mg/l"],
                "CO3": piper_pct["Alcalinidad Carbonatada (mg CaCO3/L)"].fillna(0) * 0.5998,
                "Cl": piper_pct["Cl- (mg Cl-/L)"],
                "SO4": piper_pct["SO42- (mg SO42-/L)"],
                "Label": piper_pct["acuifero"],
                "Color": piper_pct["acuifero"].map(lambda a: COLOR_ACUIFERO.get(a, "#999999")),
                "Marker": "o",
                "Size": 60,
                "Alpha": 0.85,
            ...
            with tempfile.TemporaryDirectory() as carpeta_tmp:
                ruta_base = f"{carpeta_tmp}/piper"
                triangle_piper.plot(df_wq, unit="mg/L", figname=ruta_base, figformat="png")
                st.image(f"{ruta_base}.png", use_container_width=True)

            st.caption(
                "Cada pozo aparece 3 veces (triángulo de cationes, triángulo de aniones y "
                "rombo central), coloreado según su acuífero."
            )
            ...
            with st.expander("Ver tabla de porcentajes iónicos (meq%) de estas muestras"):
                ...
                tabla_piper = piper_pct[columnas_tabla].copy()
