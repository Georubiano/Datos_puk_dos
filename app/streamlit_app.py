...
if modo_oscuro:
    st.markdown(
        ...
        /* Texto general: markdown, párrafos, spans heredan el color claro. */
        .stApp, .stApp p, .stApp span, .stApp label, .stApp li,
        .stMarkdown, .stMarkdown p { color: #f0f2f6; }

        /* Captions y notas (por defecto quedan en un gris pensado para fondo
           blanco; se aclaran para que se lean sobre fondo oscuro). */
        [data-testid="stCaptionContainer"], [data-testid="stCaptionContainer"] *,
        .stCaption, small { color: #b8c0cc !important; }

        /* Métricas (st.metric): etiqueta, valor y delta. */
        [data-testid="stMetricLabel"] { color: #b8c0cc !important; }
        [data-testid="stMetricValue"] { color: #f0f2f6 !important; }
        [data-testid="stMetricDelta"] { color: #b8c0cc !important; }

        /* Tarjetas (st.container(border=True)) y expanders. */
        [data-testid="stExpander"], [data-testid="stVerticalBlockBorderWrapper"] {
            background-color: #161a23 !important; border-color: #2a2f3a !important;
        }
        [data-testid="stExpander"] summary { color: #f0f2f6 !important; }

        /* Tablas (st.dataframe). */
        [data-testid="stDataFrame"] { background-color: #161a23; }

        /* Inputs: selectbox, multiselect, checkbox, toggle. */
        [data-baseweb="select"] > div, [data-testid="stWidgetLabel"] p {
            background-color: #161a23; color: #f0f2f6 !important;
        }
        [data-testid="stWidgetLabel"] p { background-color: transparent; }
    ...
    plt.rcParams.update({
        ...
        "axes.edgecolor": "#f0f2f6", "grid.color": "#2a2f3a",
    })
    ...
    # Colores para elementos de Matplotlib que antes estaban en negro fijo
    # (ejes, ticks, bordes de barra) y quedaban invisibles sobre fondo oscuro.
    COLOR_EJES = "#f0f2f6"
    COLOR_BORDE_BARRA = "#0e1117"
else:
    sns.set_theme(style="whitegrid", context="notebook")
...
def _estilizar_ejes(ax) -> None:
    ...
    for lado in ("bottom", "left"):
        ax.spines[lado].set_color(COLOR_EJES)
        ax.spines[lado].set_linewidth(1)
    ax.minorticks_on()
    ax.tick_params(axis="both", which="major", color=COLOR_EJES, labelsize=9)
    ax.tick_params(axis="both", which="minor", color=COLOR_EJES, length=2.5)
    ...
    for ax, acuifero_nombre in zip(axes, acuiferos_presentes or ["Raigón", "Guaraní"]):
        ...
        for fuente in pivote.columns:
            ...
            ax.bar(pivote.index, valores, bottom=abajo, color=COLOR_FUENTE.get(fuente, "#999999"),
                   edgecolor=COLOR_BORDE_BARRA, linewidth=0.5, label=fuente)
            abajo += valores
        ax.set_title(acuifero_nombre, fontweight="bold", color=COLOR_ACUIFERO.get(acuifero_nombre, COLOR_EJES))
        ax.set_xlabel("")
        ...
        with col_perfil:
            ...
            for _, capa_geo in perfil.iterrows():
                ...
                ax.bar(
                    ...
                    width=0.5, color=color, edgecolor=COLOR_BORDE_BARRA, linewidth=0.6, label=etiqueta,
                )
            ...
            ax.minorticks_on()
            ax.tick_params(axis="y", which="both", color=COLOR_EJES)
    ...
    if pozos_hq.empty:
        ...
        if con_dato > 0:
            ...
            if not valores.empty:
                ...
                sns.histplot(valores, bins=20, color="#984ea3", edgecolor=COLOR_BORDE_BARRA, ax=ax)
    ...
    if pozos_hidraulica.empty:
        ...
        with col_ficha:
            ...
            if pd.notna(profundidad_total):
                ...
                ax.bar(0, profundidad_total, width=0.4, color="#d9d9d9", edgecolor=COLOR_BORDE_BARRA)
